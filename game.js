...
function renderSceneBanner(bannerId, bgKey) {
  ...
  const drawBg = () => {
    if (img && img.complete && img.naturalWidth) {
      // Cover-fit: scale image to fill canvas, crop excess (like CSS object-fit:cover)
      const iw = img.naturalWidth, ih = img.naturalHeight;
      const scale = Math.max(cv.width / iw, cv.height / ih);
      const sw = cv.width  / scale;
      const sh = cv.height / scale;
      const sx = (iw - sw) / 2;
      const sy = (ih - sh) / 2;
      ctx.drawImage(img, sx, sy, sw, sh, 0, 0, cv.width, cv.height);
    } else {
      ctx.fillStyle = '#0a0700'; ctx.fillRect(0,0,cv.width,cv.height);
    }
