...
const bootMessages = [
  ...
  { text: 'Connecting to ByteCafé BBS v3.4...', delay: 2200, ok: true },
...
window.onload = function() {
  ...
  // PC startup sound fires immediately on load
  // Small delay so AudioContext can init after page load
  ...
  bootMessages.forEach((msg, i) => {
    setTimeout(() => {
      ...
      // Sound: dialup for BBS line, regular beep for others
...
function applyOwnedCosmetics() {
  document.body.classList.remove(
    'theme-green','theme-blue','theme-red','theme-white',
    'crt-glow','scanlines-off','scanlines-heavy','cursor-beam'
  );
  ...
  if (econ.owned['theme_red'])       document.body.classList.add('theme-red');
  if (econ.owned['theme_white'])     document.body.classList.add('theme-white');
  ...
  if (econ.owned['scanlines_heavy']) document.body.classList.add('scanlines-heavy');
  if (econ.owned['cursor_beam'])     document.body.classList.add('cursor-beam');
}
...
function unlockBytosLog() {
  bbsBoards['bytos_log'] = {
    name: '🖥️ BYTOS — INTERNAL',
    posts: [
      { author:'BYTE/OS', date:'1999-11-04 23:47:12', subject:'[session log — user REN_T]',
      ...
      { author:'BYTE/OS', date:'1999-11-04 23:58:44', subject:'[flag — external query]',
      ...
      { author:'BYTE/OS', date:'1994-08-14 09:12:03', subject:'[first boot — archived]',
...
function playTone(freq, type, duration, volume, startTime, endFreq) {
  if (_muted) return;
  try {
    ...
    if (endFreq) osc.frequency.exponentialRampToValueAtTime(endFreq, ctx.currentTime + (startTime||0) + duration);
    gain.gain.setValueAtTime(volume * _sfxVol, ctx.currentTime + (startTime||0));
...
function playNoise(duration, volume, startTime) {
  ...
  try {
    ...
    gain.gain.setValueAtTime(volume * _sfxVol, ctx.currentTime + (startTime||0));
    gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + (startTime||0) + duration);
    src.start(ctx.currentTime + (startTime||0));
  } catch(e) {}
}
...
const SFX = {
  ...
  // Keyboard type — very short soft tick, throttled to 80ms
  type() {
    const now = Date.now();
    if (now - _lastTypeTime < 80) return;
    _lastTypeTime = now;
    playTone(200 + Math.random()*100, 'square', 0.025, 0.08);
    playNoise(0.02, 0.04);
  },

  // Puzzle success — rising chime chord
  success() {
    playTone(523, 'sine', 0.15, 0.3, 0);      // C5
    playTone(659, 'sine', 0.15, 0.25, 0.05);  // E5
    playTone(784, 'sine', 0.2,  0.35, 0.1);   // G5
    playTone(1047,'sine', 0.25, 0.4,  0.18);  // C6
  },

  // Wrong answer — low buzzy descend
  error() {
    playTone(220, 'sawtooth', 0.08, 0.2, 0);
    playTone(180, 'sawtooth', 0.12, 0.2, 0.05);
    playTone(140, 'sawtooth', 0.1,  0.15, 0.12);
  },

  // Coin earned — bright little ping
  coin() {
    playTone(880, 'sine', 0.06, 0.25, 0);
    playTone(1320,'sine', 0.08, 0.1,  0.05);
  },

  // Shop purchase — satisfying cash register-ish
  purchase() {
    playTone(440, 'square', 0.04, 0.2, 0);
    playTone(660, 'square', 0.04, 0.2, 0.04);
    playTone(880, 'sine',   0.1,  0.2, 0.08);
  },

  // Badge earned — fanfare flourish
  badge() {
    playTone(523, 'sine', 0.08, 0.3, 0);
    playTone(659, 'sine', 0.08, 0.3, 0.07);
    playTone(784, 'sine', 0.08, 0.3, 0.14);
    playTone(1047,'sine', 0.15, 0.35,0.21);
    playTone(1319,'sine', 0.2,  0.4, 0.3);
  },

  // Level up — ascending arpeggio
  levelup() {
    [523,659,784,1047,1319,1568].forEach((f,i) => {
      playTone(f, 'sine', 0.12, 0.3, i*0.07);
    });
    playTone(2093, 'sine', 0.25, 0.5, 0.45);
  },

  // Boot line beep — old terminal sound
  boot() {
    playTone(440, 'square', 0.04, 0.12);
    playNoise(0.02, 0.06, 0.03);
  },

  // Full PC POST startup sound — plays once on boot screen
  pcStartup() {
    if (_muted) return;
    try {
      const ctx = getAudioCtx();

      // 1. POST beep — single short high beep (BIOS)
      playTone(1000, 'square', 0.12, 0.25, 0);

      // 2. HDD seek noise — scratchy burst
      playNoise(0.08, 0.15, 0.25);
      playNoise(0.06, 0.10, 0.36);
      playNoise(0.05, 0.12, 0.44);

      // 3. Windows 95 / DOS style startup chord
      // Root chord: C major with some warble
      const chordTime = 0.65;
      const chordNotes = [
        { freq: 523, vol: 0.18, delay: 0,    dur: 1.2 },  // C5
        { freq: 659, vol: 0.14, delay: 0.05, dur: 1.1 },  // E5
        { freq: 784, vol: 0.12, delay: 0.1,  dur: 1.0 },  // G5
        { freq: 1047,vol: 0.16, delay: 0.18, dur: 0.9 },  // C6 — the "ding"
      ];
      chordNotes.forEach(n => {
        playTone(n.freq, 'sine', n.dur, n.vol, chordTime + n.delay);
      });

      // 4. Sub-bass thud under the chord
      playTone(80, 'sine', 0.3, 0.3, chordTime);

      // 5. Tiny shimmer on top
      playTone(2093, 'sine', 0.15, 0.06, chordTime + 0.22);
      playTone(2637, 'sine', 0.12, 0.04, chordTime + 0.32);

    } catch(e) {}
  },

  // Dial-up modem connect — classic sequence
  dialup() {
    const seq = [
      [1200,0],[2000,0.08],[800,0.18],[1600,0.26],
      [600,0.36],[1800,0.44],[1000,0.54],[2200,0.62],
    ];
    seq.forEach(([f,t]) => {
      playTone(f, 'sine', 0.07, 0.2, t);
      playNoise(0.06, 0.08, t+0.01);
    });
  },

  // Window switch — soft whoosh
  whoosh() {
    playTone(300, 'sine', 0.12, 0.1, 0, 600);
  },
};

// Mute toggle — called by taskbar button
function toggleMute() {
  _muted = !_muted;
  const btn = document.getElementById('btn-mute');
  if (btn) btn.textContent = _muted ? '🔇 MUTE' : '🔊 MUTE';
  if (_muted) {
    musicPause();
  } else {
    musicResume();
  }
  showToast(_muted ? 'Audio muted.' : 'Audio on.', 'amber');
}
...
// ══════════════════════════════════════════════════════════════
const MUSIC_TRACKS = {
  cafe:    ['assets/audio/ost_cafe.mp3'],
  console: ['assets/audio/ost_console_1.mp3','assets/audio/ost_console_2.mp3',
            'assets/audio/ost_console_3.mp3','assets/audio/ost_console_4.mp3'],
  late:    ['assets/audio/ost_late_1.mp3','assets/audio/ost_late_2.mp3'],
};

// Which window/scene maps to which music pool
const WINDOW_MUSIC = {
  story:   'cafe',
  bbs:     'cafe',
  notes:   'cafe',
...
// NODE_BG keys that use late night music
const LATE_NIGHT_NODES = ['late_night', 'ch2_setup'];

let _music = {
  audio:       null,      // current HTMLAudioElement
  pool:        null,      // current pool key
  trackIndex:  0,
  fadeTimer:   null,
  volume:      0.35,      // master music volume (0–1)
};

function _pickTrack(pool) {
  const tracks = MUSIC_TRACKS[pool];
  if (!tracks || tracks.length === 0) return null;
  // Pick a random track from the pool, avoid repeating the last one if possible
  let idx = Math.floor(Math.random() * tracks.length);
  if (tracks.length > 1 && idx === _music.trackIndex) {
    idx = (idx + 1) % tracks.length;
  }
  _music.trackIndex = idx;
  return tracks[idx];
}

function musicPlay(pool, fadeMs = 800) {
  if (_muted) return;
  if (_music.pool === pool && _music.audio && !_music.audio.paused) return;

  const src = _pickTrack(pool);
  if (!src) return;

  // Hard-stop any existing audio immediately
  if (_music.audio) {
    _music.audio.pause();
    _music.audio.src = '';
    _music.audio = null;
...
function musicPause() {
  if (_music.audio) _fadeOut(_music.audio, 500, () => _music.audio && _music.audio.pause());
}

function musicResume() {
  if (_music.audio && _music.audio.paused) {
    _music.audio.play().catch(() => {});
    _fadeIn(_music.audio, 500);
  }
}

function musicSetPool(pool) {
  if (_music.pool !== pool) musicPlay(pool);
}

function _fadeIn(audio, ms) {
  const target = _music.volume;
  const steps  = 20;
...
function _fadeOut(audio, ms, onDone) {
  ...
  const t = setInterval(() => {
    try {
      ...
      if (audio.volume <= 0) { clearInterval(t); if (onDone) onDone(); }
    } catch(e) { clearInterval(t); if (onDone) onDone(); }
  }, interval);
}
...
function startGame() {
  ...
  loadSceneAssets().then(() => {
    ...
    // Start café music after a short delay (lets browser unblock autoplay via user gesture)
    setTimeout(() => musicPlay('cafe', 1200), 400);
  });
}

// Timer removed — game is untimed
...
function switchWindow(name) {
  ...
  SFX.whoosh();
  // Music: switch pool based on window
  const pool = WINDOW_MUSIC[name] || 'cafe';
  musicSetPool(pool);
}
...
function renderSceneBanner(bannerId, bgKey) {
  ...
  // Match canvas resolution exactly to the rendered banner size
  // Use rect for precise dimensions after layout
  const rect = banner.getBoundingClientRect();
  const W = Math.round(rect.width)  || 700;
  const H = Math.round(rect.height) || 110;
  cv.width  = W;
  cv.height = H;
  cv.style.cssText = 'width:100%;height:100%;display:block;position:absolute;top:0;left:0;';
  ...
  const drawBg = () => {
    if (img && img.complete && img.naturalWidth) {
      // Cover-fit: fill banner width, crop height from center
  ...
  if (!img) { startAnim(); return; }
  if (img.complete && img.naturalWidth) { startAnim(); return; }
  img.onload = startAnim;
  setTimeout(startAnim, 500); // fallback
}

// cafe_main — monitor screen flickers + subtle warm pulse
function animCafeMain(ctx, cv, drawBg) {
  // Monitor positions in the image (approximate as % of canvas)
  const monitors = [
    { x:0.08, y:0.30, w:0.12, h:0.18 }, // front left monitor
    { x:0.25, y:0.28, w:0.10, h:0.16 }, // row back left
    { x:0.38, y:0.27, w:0.10, h:0.16 }, // row back mid
    { x:0.52, y:0.27, w:0.10, h:0.16 }, // row back mid-right
    { x:0.68, y:0.27, w:0.10, h:0.16 }, // row back right
    { x:0.82, y:0.27, w:0.10, h:0.16 }, // far right
  ];
  // Each monitor gets its own flicker timer
  const states = monitors.map(() => ({ v:0.8+Math.random()*0.2, t:0, next:20+Math.random()*60 }));
  let t=0;
  ...
  function frame() {
    ...
    monitors.forEach((m, i) => {
      ...
      s.t++;
      if (s.t >= s.next) {
        // Random flicker: dim briefly then recover
        s.v = 0.2 + Math.random() * 0.6;
        s.t = 0;
        s.next = 40 + Math.random() * 120;
      } else {
        s.v = Math.min(1, s.v + 0.04); // recover toward bright
      }
      // Blue-white screen glow overlay on monitor area
      ctx.fillStyle = `rgba(100,150,255,${0.08 * s.v})`;
...
// terminal_7 — green CRT monitor flicker + steam from coffee
function animTerminal7(ctx, cv, drawBg) {
  // The big CRT screen is roughly center-left of the image
  const screen = { x:0.13, y:0.02, w:0.52, h:0.75 };
  let screenV = 1.0, screenT = 0, screenNext = 80;
  let glow = 0;

  // Steam particles rising from coffee cup (right side)
  const steam = Array.from({length:12}, (_,i) => ({
    x: cv.width*0.76 + (Math.random()-0.5)*14,
    y: cv.height*0.52 - i*5,
  ...
  function frame() {
    drawBg(); glow += 0.02;
    screenT++;
    if (screenT >= screenNext) {
      screenV = 0.3 + Math.random() * 0.5;
      screenT = 0;
      screenNext = 60 + Math.random() * 140;
    } else {
      screenV = Math.min(1, screenV + 0.03);
    }
    // Green phosphor glow over CRT screen area
    ctx.fillStyle = `rgba(60,200,80,${0.07 * screenV + Math.sin(glow)*0.015})`;
    ctx.fillRect(screen.x*cv.width, screen.y*cv.height, screen.w*cv.width, screen.h*cv.height);
    // Scanline sweep
    const scanY = ((glow * 30) % cv.height);
    ctx.fillStyle = `rgba(100,220,100,${0.04 * screenV})`;
    ctx.fillRect(screen.x*cv.width, scanY, screen.w*cv.width, 3);

    // Steam
    steam.forEach(p => {
      ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI*2);
      ctx.fillStyle = `rgba(210,185,145,${p.op})`; ctx.fill();
      p.x += p.vx + Math.sin(glow + p.y*0.04)*0.2;
      p.y += p.vy; p.op -= 0.0006;
      if (p.op <= 0 || p.y < -8) {
        p.x = cv.width*0.76 + (Math.random()-0.5)*14;
        p.y = cv.height*0.54; p.op = 0.07+Math.random()*0.09;
...
// corkboard — warm light pulse + floating dust motes
function animCorkboard(ctx, cv, drawBg) {
  const motes = Array.from({length:18}, () => ({
    x: Math.random()*cv.width, y: Math.random()*cv.height,
    vx: (Math.random()-0.5)*0.2, vy: -0.06-Math.random()*0.14,
    op: 0.05+Math.random()*0.14, r: 0.5+Math.random()*1.2,
    ph: Math.random()*Math.PI*2,
  }));
  let t=0;
  function frame() {
    drawBg(); t+=0.015;
    // Warm light pulse from the bottom (lamp glow)
    const pulse = 0.03 + Math.sin(t*0.8)*0.015;
    ctx.fillStyle = `rgba(255,160,60,${pulse})`;
    ctx.fillRect(0, cv.height*0.5, cv.width, cv.height*0.5);
    motes.forEach(m => {
      m.x += m.vx + Math.sin(t+m.ph)*0.1; m.y += m.vy;
      if (m.y < -5) { m.y = cv.height+5; m.x = Math.random()*cv.width; }
      ctx.beginPath(); ctx.arc(m.x, m.y, m.r, 0, Math.PI*2);
      ctx.fillStyle = `rgba(255,210,130,${m.op})`; ctx.fill();
    });
    return requestAnimationFrame(frame);
  }
  return frame();
...
// late_night — green monitor flicker + lamp flicker + moth
function animLateNight(ctx, cv, drawBg) {
  // The one glowing green monitor in the image
  const greenMon = { x:0.38, y:0.26, w:0.10, h:0.16 };
  let monV=1, monT=0, monNext=50;
  let lampV=1, lampT=0, lampNext=40;
  const moth = { ph:0, r:14 };

  function frame() {
    drawBg();

    // Green monitor flicker
    monT++;
    if (monT >= monNext) { monV=0.1+Math.random()*0.7; monT=0; monNext=30+Math.random()*100; }
    else monV = Math.min(1, monV+0.05);
    ctx.fillStyle = `rgba(60,220,80,${0.18*monV})`;
    ctx.fillRect(greenMon.x*cv.width, greenMon.y*cv.height, greenMon.w*cv.width, greenMon.h*cv.height);

    // Lamp flicker — amber radial glow right side
    lampT++;
    if (lampT >= lampNext) { lampV=0.4+Math.random()*0.6; lampT=0; lampNext=35+Math.random()*65; }
    else lampV = Math.min(1, lampV+0.06);
    const g = ctx.createRadialGradient(cv.width*0.74, cv.height*0.3, 0, cv.width*0.74, cv.height*0.3, cv.width*0.28);
    g.addColorStop(0, `rgba(232,155,30,${0.18*lampV})`);
    g.addColorStop(1, 'rgba(232,155,30,0)');
    ctx.fillStyle = g; ctx.fillRect(0,0,cv.width,cv.height);

    // Moth circling lamp
    moth.ph += 0.035;
    const mx = cv.width*0.74 + Math.cos(moth.ph)*moth.r;
    const my = cv.height*0.30 + Math.sin(moth.ph*1.4)*moth.r*0.45;
    ctx.beginPath(); ctx.arc(mx, my, 1.3, 0, Math.PI*2);
    ctx.fillStyle = `rgba(255,215,150,${0.5*lampV})`; ctx.fill();
    ctx.beginPath(); ctx.ellipse(mx-2.2, my, 2.2, 1.0, moth.ph, 0, Math.PI*2);
    ctx.fillStyle = `rgba(195,170,100,${0.32*lampV})`; ctx.fill();
    ctx.beginPath(); ctx.ellipse(mx+2.2, my, 2.2, 1.0, -moth.ph, 0, Math.PI*2); ctx.fill();
...
function loadNode(nodeId) {
  ...
  // Music: late night nodes get the late pool, others get cafe
  const musicPool = bgKey === 'late_night' ? 'late' : 'cafe';
  musicSetPool(musicPool);
...
function checkPuzzle() {
  ...
  if (p.check(sessionCode, sessionOutputs)) {
    // ── SUCCESS ──────────────────────────────────────────────
    SFX.success();
    // Brief victory sting — play over current music then return
    if (!_muted) {
    ...
    // Permanent XP multiplier upgrade
    ...
    let repReward  = PUZZLE_REP[idx]   || 1;
    // Permanent REP boost upgrade
...
function showAllComplete() {
  ...
  appendOutput('r-result success', '');
  appendOutput('r-result success', '★ ★ ★  ALL PUZZLES COMPLETE  ★ ★ ★');
  appendOutput('r-result success', '');
...
function renderShop() {
  ...
  cats.forEach(cat => {
    ...
    SHOP_ITEMS.filter(i => i.category === cat.key).forEach(item => {
      const isOwned = (item.type === 'cosmetic' || item.type === 'story' || item.type === 'upgrade' || item.type === 'bytos_mode' || item.type === 'name_tag' || item.type === 'custom_boot')
        ? !!econ.owned[item.unlock]
        : false;
      // Apply hint discount upgrade
      const displayCost = (item.type === 'hint' && econ.owned['hint_discount']) ? 0 : item.cost;
      const effectiveCost = displayCost;
      const cantAfford = econ.coins < effectiveCost || (item.repCost && econ.rep < item.repCost);
      const el = document.createElement('div');
      ...
      if (!isOwned && !cantAfford) el.onclick = () => { const i2 = {...item, cost: effectiveCost}; buyItem(i2); };
      grid.appendChild(el);
    });
