...
function applyUpgrades() {
  // extended_session — only applies on fresh start, handled in initPuzzles
...
function musicPlay(pool, fadeMs = 800) {
  if (_muted) return;
  if (_music.pool === pool && _music.audio && !_music.audio.paused) return; // already playing

  const src = _pickTrack(pool);
  if (!src) return;

  // Fade out existing track
  const old = _music.audio;
  if (old) {
    _fadeOut(old, fadeMs, () => { old.pause(); old.src = ''; });
  }
  ...
  a.loop   = true;
  a.volume = 0;
  a.play().catch(() => {}); // autoplay may be blocked — silent fail
  _fadeIn(a, fadeMs);
...
function initPuzzles() {
  // Apply extended session upgrade
  if (econ.owned['extended_session'] && timeLeft === 28 * 60) {
    timeLeft = 45 * 60;
  }
...
const SHOP_ITEMS = [
  ...
  { id:'time_5',      category:'consumable', icon:'⏳',    name:'+5 MIN TIME',
    desc:'Five more minutes. Mr. K does not question it.',
  ...
  { id:'extended_session', category:'upgrade', icon:'🕐', name:'EXTENDED SESSION',
    desc:'Start every session with 45 minutes instead of 28. More time, less panic.',
    cost:5, repCost:0, type:'upgrade', unlock:'extended_session' },
